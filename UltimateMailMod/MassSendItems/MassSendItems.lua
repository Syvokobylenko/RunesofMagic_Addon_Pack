...
function UMMMassSendItemsStatusTemplate_OnHide(this)
  ...
  getglobal(this:GetParent():GetName().."RecipientRecipient"):SetFocus();
  this:AutoEnableTabs();
end
...
function UMMMassSendItemsRecipientTemplate_OnLoad(this)
  ...
  this.Reset = function(self)
    ...
    getglobal(this:GetName().."Recipient"):SetFocus();
  end;
...
function UMMMassSendItemsMarkButtonsTemplate_OnLoad(this)
  ...
  this.ButtonClick = function(self, id)
	if (id == 28) then
	 for xxxx = 1, 27 do
		local type = UMMItemDB:GetTypeByID(xxxx);
		if (getglobal("UMM_MSI_RECIPE_SEND_TO"..id) ~= "") or (getglobal("UMM_MSI_RECIPE_SEND_TO"..id) ~= nil) then
			getglobal(this:GetParent():GetName().."RecipientRecipient"):SetText(getglobal("UMM_MSI_RECIPE_SEND_TO"..id));
		end
		for index = 1, 180 do
		  if (UMMBagManager.ItemList[index].Empty == true) then
			-- Skip empty slots
		  else
			local itemColor = string.sub(UMMBagManager.ItemList[index].Link, string.find(UMMBagManager.ItemList[index].Link, "|c%x%x%x%x%x%x%x%x") + 4, string.find(UMMBagManager.ItemList[index].Link, "|c%x%x%x%x%x%x%x%x") + 9);
			local R, G, B = tonumber(string.sub(itemColor, 1, 2), 16), tonumber(string.sub(itemColor, 3, 4), 16), tonumber(string.sub(itemColor, 5, 6), 16);
			if (R == 255 and G == 255 and B == 255) and (type == "WRecipes") then								-- release 5.0.1.2553
			  -- Sol.string.StartsWith(wo,was) => string.sub(wo, 1, string.len(was)) == was
			...
			elseif (R == 0 and G == 255 and B == 0) and (type == "GRecipes") then								-- release 5.0.1.2553
			  if(Sol.string.StartsWith(UMMBagManager.ItemList[index].Name,UMM_RECIPES_NAME_PREFIX)) then		-- release 5.0.1.2553
				UMMBagManager.ItemList[index].Marked = true;													-- release 5.0.1.2553
			  end																								-- release 5.0.1.2553
			elseif (R == 0 and G == 114 and B == 188) and (type == "BRecipes") then								-- release 5.0.1.2553
			  if(Sol.string.StartsWith(UMMBagManager.ItemList[index].Name,UMM_RECIPES_NAME_PREFIX)) then		-- release 5.0.1.2553
				UMMBagManager.ItemList[index].Marked = true;													-- release 5.0.1.2553
			  end																								-- release 5.0.1.2553
			...
			elseif (UMMItemDB:IsItem(type, UMMBagManager.ItemList[index].Link)) then	-- check database
			  UMMBagManager.ItemList[index].Marked = true;
			end
		  end
		end
      ...
      local type = UMMItemDB:GetTypeByID(id);		-- type = literal string (eg. "runes")
	  if (getglobal("UMM_MSI_RECIPE_SEND_TO"..id) ~= "") or (getglobal("UMM_MSI_RECIPE_SEND_TO"..id) ~= nil) then				-- release 5.0.3.2579
	     getglobal(this:GetParent():GetName().."RecipientRecipient"):SetText(getglobal("UMM_MSI_RECIPE_SEND_TO"..id));		-- release 5.0.3.2579
	  end
      for index = 1, 180 do
        if (UMMBagManager.ItemList[index].Empty == true) then
          -- Skip empty slots
  ...
  local shiftX = 45;
  for index = 1, 28 do			-- release 4.0.11.2531 -- credit Ultak
