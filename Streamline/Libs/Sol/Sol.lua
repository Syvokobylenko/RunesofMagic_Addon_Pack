...
--]]
Sol.config.RestoreFrameBounds = function(addonName, frame)
    local addon = Sol._help.CheckAddon(addonName)
...
Parameters:
+ bagIndex - 1 for Bag I, 2 for Bag II, etc
...
Sol.data.items.FindEmptyBagSlot = function(bagIndex)
    local maxItems = BAG_MAX_ITEMS
    local offset = 0
    if not bagIndex then
        maxItems = BAG_MAX_ITEMS * 2
    else
        offset = bagIndex * BAG_MAX_ITEMS
    end
...
Sol.hooks.GetOriginalFn = function(addonName, fnName, frame)
    ...
    local tableName = (frame and frame ~= _G and frame.GetName and frame:GetName()) or "_G"
...
things up you'll most likely have to restart the game. 
--]]
Sol.hooks.Hook = function(addonName, fnName, newFn, frame)
    ...
    if newFn == tbl[fnName] or not tbl[fnName] or not newFn then
        return false
    end
...
Sol.math.DecToHex = function(dec)
    ...
    while dec > 0 do
        ...
        hex = string.sub(hexValues, mod, mod) .. hex
