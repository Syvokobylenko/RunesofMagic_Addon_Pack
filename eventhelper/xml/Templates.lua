function EventHelper_Tab_Template_OnClick(this, override)
...
function EventHelper_Tab_Template_OnEnter(this)
...
function EventHelper_Tab_Template_OnShow(this)
	...
	while _G[frame .. "Tab" .. num] do
		...
		if tab.frame and type(_G[frame].numTabs) == "number" then
			if _G[tab.frame] and tab.visibleOnShow and tab.visibleOnShow == true then
				EventHelper_Tab_Template_OnClick(tab, true)
			end
		end
		num = num + 1
	end
...
function EventHelper_button_SetTab(this, tbl)
	if this then
		local frame = this:GetName()
		local num = 1
